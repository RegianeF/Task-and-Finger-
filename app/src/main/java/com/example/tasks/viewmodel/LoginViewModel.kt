...
class LoginViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun isAuthenticationAvailable() {
        ...
        if (FingerPrintHelper.isAuthenticationAvaliable(getApplication())) {
            //alguma maneira de notificação a LoginActivity está disponivel pq é lá em showAutentication que mostra a autenticação:
            mFingerprint.value =
                everLogged //notifica se o usuario já foi salvo alguma vez como login
        }
    }
}